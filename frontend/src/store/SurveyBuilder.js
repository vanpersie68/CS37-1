...
export default new Vuex.Store({
  state: {
    mutex: false,
    survey: {},
    random: localStorage.getItem('random') == 'true' ? true : false,
  ...
  actions: {
    ...
    async updateBlockQ({ commit },payload){
      console.log(payload)
      if(payload.flag){
        const res = await SurveyServices.postBlockQ(payload.bid, { q: parseInt(payload.q), flag: 1 })


      } else {
        const res = await SurveyServices.postBlockQ(payload.bid, { q: parseInt(payload.q), flag: 0 })
    ...
    openRandom({ state }) {
      ...
      localStorage.setItem('random', state.random)
    },
    ...
    async fetchVideoData(state, { resp, question }) {
     ...
     async updateBlock({  getters }, { block }) {
      const currentBlockIndex = getters.sortedBlocks.findIndex(
        (el) => el.id == block.id
      )
      getters.sortedBlocks[currentBlockIndex] = { ...block }
    },
  },
  getters: {
