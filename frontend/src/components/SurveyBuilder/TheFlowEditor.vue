        ...
        <div v-else class="block">
          ...
          >
            ...
            >
             ...
             <div class="show-row" v-if="isButtonToggled">
                <span>Show:</span>
                <el-switch
                    :value="switchList[question.id]"
                    @change="(e) => { switchChange(e, question, index) }"
                >
                </el-switch>
              </div>
            </div>
...
export default {
  ...
  computed: {
   isButtonToggled() {
      return this.$store.state.isButtonToggled;
    },
    ...mapGetters([
      'sortedBlocks',
      'questionTypes',
    ...
    orderedFlow() {
      ...
      flow.map((el) => {
        el.questionData &&
          el.questionData.questions &&
          el.questionData.questions.map((ele) => {
            if (!ele.visible) {
              ele.visible = false
            } else {
              ele.visible = true
            }
            this.switchList[ele.id] = ele.visible
          })
      })
  ...
  methods: {
   async switchChange(e, question, index) {
      this.$set(this.switchList, question.id, e)
      let flow = JSON.parse(JSON.stringify(this.sortedBlocks))
      const findIdex = flow[index].questionData.questions.findIndex(
        (el) => el.id == question.id
      )
      flow[index].questionData.questions[findIdex].visible = e ? 1 : 0
      this.$forceUpdate()
      await store.dispatch('updateBlock', { block: flow[index] })
