...
class BlockSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Block
        fields = ['id', 'survey', 'title', 'description', 'order','q','type','selected_ready','flag']
...
class QuestionSerializer(serializers.ModelSerializer):
    order = serializers.IntegerField(max_value=2147483647, min_value=0, required=True)
    visible = serializers.IntegerField(max_value=2147483647, min_value=0, required=False)

    class Meta:
        model = Question
        fields = ['id', 'block', 'name', 'type', 'description', 'required', 'order', 'image','visible']
